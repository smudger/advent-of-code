...
solve :: String -> Int
solve x = length . filter (isLoop x) . drop 1 . nub . mapRoute $ x
...
isLoop :: String -> Point -> Bool
isLoop x o = isLoop' (o : obstacles) dimension (startPoint, North) []
  where
    obstacles = map fst . filter ((== '#') . snd) . toGrid $ x
    startPoint = fst . fromJust . find ((== '^') . snd) . toGrid $ x
    dimension = length . lines $ x

isLoop' :: [Point] -> Int -> (Point, Direction) -> [(Point, Direction)] -> Bool
isLoop' obstacles dimension v@((x, y), _) seen
  | x < 0 || y < 0 || x >= dimension || y >= dimension = False
  | v `elem` seen = True
  | otherwise = isLoop' obstacles dimension (hop obstacles v) (v : seen)
