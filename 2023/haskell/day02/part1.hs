...
isValidCount :: (Int, Int, Int) -> Bool
isValidCount (r, g, b) = r <= 12 && g <= 13 && b <= 14

validGameIds :: [String] -> [Int]
validGameIds = map fst . filter (isValidCount . snd) . zip [1 ..] . map colorCounts

main = do
  input <- readFile "input.txt"
  print . sum . validGameIds . lines $ input
